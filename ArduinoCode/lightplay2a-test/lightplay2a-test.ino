          ...
          switch(ybits)
            {
              ...
              case 4: //set motorspeed slow
                ...
                pwm.setPWM(motore, 0, 500); // this is the correct way to initialize motor duty cycle to 100%
                break;

              case 5: //set motorspeed faster
                pwm.setPWM(motore, 0, 1000); // this is the correct way to initialize motor duty cycle to 100%
                break;
    ...
    if ((xbits == 3) || (xbits == 0))
      {
        for (int i = 0; i <= 3; i++)
          {
            if (!light2_is_on)
              {pwm.setPWM(11 - i, 0, RGBWtable[4 * light1color + i]);}
            else
              {pwm.setPWM(11 - i, 0, 4096);}
          }
