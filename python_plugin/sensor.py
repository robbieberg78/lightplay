...
class PromptingEdgeTriggeredSensor(EdgeTriggeredSensor):
   ...
   def poll(self):
      command = raw_input("What state should {0} be in?  'H' for high, 'L' for low, any other key to not change: ".format(self._name))
      if command == "H":
         self.high_state()
      if command == "L":
         self.low_state()
...
class ArduinoEdgeTriggeredSensor(EdgeTriggeredSensor):
   ...
   def poll(self):
      self.high_state() if self._arduino.query(self._channel) else self.low_state()
      EdgeTriggeredSensor.poll(self)
