...
def connection():
    ...
    x = s.read(1) # read one byte response from Arduino, which will be '0' if button is pressed and '1'
    # confusing point: the Arduino is actually sending binary reps of 0 and 1, but serial.read expects ascii
...
# This trigger can be started from the command window by typing "trigger.start()"
